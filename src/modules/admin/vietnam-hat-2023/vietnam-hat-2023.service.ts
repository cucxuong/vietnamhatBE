...
export class VietnamHat2023Service {
  ...
  async getPlayerList({country}: { country: string | null }) {
    ...
    const players = await this.tournamentPlayerModel.find(condition).sort({status: -1, created_at: -1}).exec();
  ...
  async updatePlayerStatus({player_code, status}: { player_code: string, status: string }) {
